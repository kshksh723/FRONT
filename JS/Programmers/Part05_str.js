...
str.substring(2, 4);
// "cd"
// 문자열 인덱스가 2(세 번째 문자열)인 c 부터 인덱스가 4(다섯번째 문자열)인 e 전까지를 반환한다
// 즉, c 부터 시작해서 e 바로 앞의 문자열인 d까지 반환한담
str.substr(2, 4);
// "cdea"
// 세번째 문자열부터 길이가 4인 부분 문자열을 반환한다
// 즉, c 부터 길이가 4에 해당하는 부분 문자열인 a까지를 반환한다

// 두 명령 모두 두번째 인수를 생략할 수 있다. 인수를 생략하면 문자열의 마지막 까지를 돌려준다

str.substring(2);
// "cdeabcde"
// c 부터 문자열의 맨 끝인 e 까지를 반환한다
str.substr(2);
//"cdeabcde"
// c부터 맨끝인 e까지를 반환한다

// 두 번째 인수를 생략했을 때는 substr()과 substring()의 명령결과가 같다

// substr()명령은 첫번째 인수에 음수를 입력하면 문자열 뒤에서부터 계산하여 부분 문자열을 돌려줍니다

str.substr(-7); // 뒤에서부터 일곱번째까지의 문자열을 반환
// "deabcde"
str;
// "abcdeabcde"
str.substr(-7, 2); // 두번째 인수를 이용해서 문자열의 길이를 제한할 수 있다
// "de"

// 문자열 안에서 주어진 문자열의 위치를 찾아서 반환하는 명령도 있다
// indexOf()와 lastIndexOf() 명령을 사용하면 해당 문자열이 있는 곳의 시작 위치를 반환한다

// indexOf() 명령은 해당 문자열이 처음으로 나오는 곳의 위치
// lastIndexOf() 명령은해당 문자열이 마지막으로 나오는 곳의 시작 위치를 반환한다

// 두 명령 모두 인수에 해당 문자열에 없는 문자열을 입력하면 -1을 반환한다
str.indexOf("bc");
// 1
str.lastIndexOf("bc");
// 6
str.lastIndexOf("f"); // 없는 문자열을 입력한 경우
// -1  --> -1을 반환

/*   요약
- 특정 위치의 문자열을 알아낼 때는 charAt() 명령을 사용한다
- 대괄호를 사용해서 해당 문자열의 위치를 직접 지정할 수도 있다
- 부분문자열을 구하는 명령에는 substring()과 substr()이 있다
- 문자열을 검색하는 명령에는 indexOf()와 lastIndexOf()가 있다
